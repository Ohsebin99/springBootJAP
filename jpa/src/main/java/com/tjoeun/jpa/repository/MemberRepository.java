...
//	findById() 메소드와 유사하에 id 필드 대신 다른 필드를 이용해서 검색하는 기능을 구현하려면
...
//	First를 사용하면 검색 결과의 앞 부터 First 뒤에 지정한 숫자 크기만큼 데이터를 얻어왔다.
//	만약 지원되지는 않지만 First 대신 Last를 사용한다면 Last가 무시되고 findByName(String name) 메소드와
//	같은 기능이 실행된다.
...
//	메소드 명명 규칙: findBy필드명Between
	List<Member> findByIdBetween(Long start, Long end);
...
//	findBy필드명IsEmpty, findBy필드명IsNotEmpty는 collection이 비어있나 비어있지 않나 비교한다.
	
//	in, not in
//	메소드 명명 규칙: findBy필드명In, findBy필드명NotIn
	List<Member> findByNameIn(List<String> names);
	List<Member> findByEmailNotIn(List<String> emails);
